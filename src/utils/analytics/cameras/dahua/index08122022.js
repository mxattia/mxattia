...
var dahua = function(options) {
 ...
 var errorNumber=0; 
  ...
  if( options.cameraAlarms ) { this.client = this.connect(options,eventNames,errorNumber) };

  this.on('error',function(err){
    console.log("Error: after client  " + err);
  });
...
// The channel number is default 0 if the request is not carried the param.
 ...
 var getSnapshot = function (self,options,ch) {
  ...
  if (words.indexOf(ch.toString().trim()) ==-1){
			return ;
	}
 ...
 //  var options = {};
  ...
  var TimeStamp=createTimeStamp() ;	
   var dest= globalConfig.default.recording.locationPath+'/'+options.mokedccode+"/"+ch.toString()+"/"+ TimeStamp;
