...
var dahua = function(options) {
  ...
  this.TRACE = globalConfig.default.analytics.trace;
 ...
 var errorNumber=0; 
  ...
  try 
  {
      this.client = this.connect(options,eventNames,errorNumber)  ;
...
dahua.prototype.connect = function(options,eventNames,errorNumber) {
    ...
    if (data[id].startsWith('Code=')) {
	  ...
	  //snapshot
		...
		if (action=='Stop'){
           ...
           getSnapShot( '1200',rtspUrl(self,index.toString().trim()) ,index.toString().trim(),self);
		}
...
function getSnapShot( Width,input,channel, self) {
	...
	if (TRACE)  console.log(dest+"/"+channel+'ffdh.jpeg');
	...
	//CommandArgs.push(channel+'ffhik.jpeg');
    CommandArgs.push(dest+"/"+channel+ms+'dh.jpeg');
    ...
    var args = [
        ...
        */
        const args = [];
         ...
         args.push('-vf');
         args.push('mestimate=epzs:mb_size=16:search_param=32,codecview=mv=pf+bf');
         ...
         args.push('-preset');
         args.push('ultrafast');
              //ffmpeg -i "rtsp://moked:moked12345@62.219.161.149:554/cam/realmonitor?channel=4&subtype=1" -vf mestimate=epzs:mb_size=16:search_param=32,codecview=mv=pf+bf+bb 1.mp4  
         ...
         args.push('-an');         
         args.push(dest+"/"+channel+ms+'dh.mp4' );
    ...
    const Process1 =spawn('ffmpeg', args);
...
{
 var  URL = generateBaseUrl(self.options.user, self.options.pass, self.options.host, self.options.rtspport, "rtsp") + "/cam/realmonitor?channel=" + channel + "&subtype=1";   
